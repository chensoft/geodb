...
pub static CURRENCIES: LazyLock<IndexMap<String, Currency>> = LazyLock::new(|| {
    let mut csv = csv::Reader::from_path("res/currency.csv").expect("missing currency file");
...
{{/each}}
    ...
    std::fs::write("out/currency.rs", template.render("default", &indexmap! { "currencies" => currencies})?)?;
